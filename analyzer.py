"""Detect high-conviction DCA accumulation on sub-$50M tokens.

Only fires on $150K+ DCA orders — the kind of capital deployment
...
def analyze_dca_order(order: dict, token_data: dict | None) -> list[Signal]:
    ...
    Filters:
    - Token mcap must be under $50M (covers $5M-$50M range)
    - Single order must be >= $150K, OR
    - Multiple orders on same token totalling >= $150K in 24h
    """
    signals = []
    ...
    # Hard filter: only sub-$50M mcap tokens
    if mcap > 0 and mcap > config.MAX_MARKET_CAP:
        return signals

    # --- Signal 1: Mega DCA ---
    # Single order >= $150K on a sub-$50M token. Someone is defending a level
    ...
    if in_amount_usd >= config.MIN_DCA_VALUE_USD:
        ...
        severity = "critical" if in_amount_usd >= 500_000 else "high"
    ...
    # Multiple wallets stacking DCA on the same token, total >= $150K in 24h.
    ...
    if total_value >= config.COORDINATED_MIN_VALUE_USD:
        ...
        if order_count >= 2 and unique_wallets >= 2:
            ...
            severity = "critical" if total_value >= 500_000 or unique_wallets >= 4 else "high"
