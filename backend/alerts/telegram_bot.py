...
async def fire_wallet_alert(activity: dict):
    """
    Fire a wallet tracker alert — only for truly high-priority moves.
    Does NOT count toward the daily limit (these are rare and important).
    Still requires the token to have data if it's a new accumulation.
    """
    db = SessionLocal()
    try:
        # For new token accumulations, check if we have data on the token
        if activity.get("is_new_token") and activity.get("token_contract"):
            flagged = db.query(FlaggedToken).filter_by(
                contract_address=activity["token_contract"]
            ).first()
            if flagged and not has_complete_data(flagged):
                logger.info(f"Skipping wallet alert — token {activity.get('token_symbol')} has no data yet")
                # Log to DB but don't send
                alert = Alert(
                    contract_address=activity.get("token_contract", ""),
                    token_symbol=activity.get("token_symbol", ""),
                    alert_type="wallet_tracker",
                    trigger_reason=f"Known operator {activity.get('wallet_label', '')} — {activity['activity']}. Pending data enrichment.",
                    telegram_sent=False,
                    fired_at=datetime.utcnow(),
                )
                db.add(alert)
        ...
        # Format and send
        wallet_label = activity.get('wallet_label', 'Unknown Operator')
        token_sym = activity.get('token_symbol', 'BNB')
        activity_type = activity['activity']

        message = f"""\U0001f6a8 <b>KNOWN OPERATOR ALERT</b>

<b>{wallet_label}</b>
Wallet: <code>{activity['wallet'][:10]}...</code>
...
"""
        if activity.get('is_new_token'):
            ...
            message += "\n\nThis operator was involved in a previous confirmed pump and is now buying a new token."
        ...
        alert = Alert(
            ...
            token_symbol=activity.get("token_symbol", ""),
            alert_type="wallet_tracker",
            trigger_reason=f"Known operator {wallet_label} — {activity_type}",
            telegram_sent=msg_id is not None,
            telegram_message_id=msg_id,
            fired_at=datetime.utcnow(),
        )
        ...
        logger.error(f"Error firing wallet alert: {e}")
    finally:
        db.close()
